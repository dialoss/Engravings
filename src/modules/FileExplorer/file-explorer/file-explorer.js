...
(function() {
	...
	window.FileExplorer = function(parentelem, options) {
		if (!(this instanceof window.FileExplorer))  return new window.FileExplorer(parentelem, options);
		...
		var ThumbnailDisplayHandler = function(opts, success) {
			...
			else
			{
				...
				if ((opts.id in queuedthumbs) && (queuedthumbs[opts.id] in entryidmap))
				{
					...
					node.className = 'custom-icon custom-icon-img';
		...
		var SyncCurrFolderToDOM = function() {
			...
			for (; x < entries.length; x++)
			{
				...
				{
					itemicon.className = 'custom-icon fiv-cla';
					...
					if (entries[x].type === 'folder')
					{
						...
						itemicon.classList.add('fiv-icon-folder');
					}
					else
					{
						var ext = entries[x].name;
						var pos = ext.lastIndexOf('.');
						if (pos < 0)  itemicon.classList.add('fiv-icon-file');
						else
						{
							...
							if (ext === '')  itemicon.classList.add('fiv-icon-file');
							else
							{
								var cc = ext.charCodeAt(0);

								if ((cc >= 48 && cc < 58) || (cc >= 64 && cc < 91))  itemicon.classList.add('fiv-icon-' + ext.toLowerCase());
							}
						}
					}
